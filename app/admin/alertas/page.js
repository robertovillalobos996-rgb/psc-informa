  ...
  return (
    <div className="p-10 text-black bg-white rounded-3xl shadow-xl max-w-2xl mx-auto mt-10">
      ...
      <div className="grid grid-cols-2 gap-4 mb-10">
        <button onClick={() => ejecutarRobot('nacionales')} className="p-5 bg-blue-600 text-white font-bold rounded-xl shadow-lg">NOTICIAS CR</button>
        <button onClick={() => ejecutarRobot('internacionales')} className="p-5 bg-indigo-600 text-white font-bold rounded-xl shadow-lg">MUNDO</button>
      </div>
      <form onSubmit={enviarAlerta} className="space-y-4">
        <input value={mensaje} onChange={(e) => setMensaje(e.target.value)} className="w-full p-4 border-2 rounded-xl font-bold" placeholder="MENSAJE DE ALERTA" />

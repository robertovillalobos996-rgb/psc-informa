...
export default function AdminNoticias() {
  ...
  const publicar = async (e: any) => {
    e.preventDefault();
    setStatus('📡 PROCESANDO NOTICIA...');
    
    try {
      let finalUrl = '';
      if (archivo) {
        // Limpieza de nombre: quita espacios y tildes para que Supabase no falle
        ...
        if (upErr) {
          setStatus('❌ ERROR AL SUBIR IMAGEN: ' + upErr.message);
          return;
        }
      ...
      setStatus('✅ NOTICIA PUBLICADA CON ÉXITO');
      setTitulo(''); setContenido(''); setArchivo(null); setManual(false);
      
      const fileInput = document.getElementById('file_noticia') as HTMLInputElement;
  ...
  return (
    <div className="p-8 bg-white rounded-[2rem] text-black shadow-2xl border-4 border-slate-100">
      <h2 className="text-xl font-black mb-6 uppercase italic border-l-4 border-black pl-4">Redacción de Noticias</h2>
      <form onSubmit={publicar} className="space-y-4">
        ...
        <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-xl border-2">
           ...
           <label className="font-black text-xs uppercase italic">Mostrar en Carrusel Principal</label>
        </div>

        <div className="p-6 border-4 border-dashed rounded-2xl bg-slate-50 text-center">
           <label className="block mb-2 font-black text-[10px] uppercase tracking-widest text-slate-400">Adjuntar Foto o Video:</label>
           <input id="file_noticia" type="file" onChange={(e: any) => setArchivo(e.target.files[0])} className="w-full text-xs" />
        </div>

        <button className="w-full bg-black text-white py-5 rounded-2xl font-black uppercase shadow-xl hover:bg-red-600 transition-all">PUBLICAR AHORA</button>
